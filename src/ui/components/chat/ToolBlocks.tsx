...
export const ToolExecutionBlock = memo(function ToolExecutionBlock({ name, status, input, output, logs = [] }: ToolExecutionBlockProps) {
  ...
  return (
    <section className="max-w-4xl px-1 py-2">
      <button
        type="button"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <span className={`text-sm font-medium ${statusToneClass}`}>{name}</span>
          {summary && !expanded ? <span className="text-sm text-muted ml-2">— {summary}</span> : null}
        </div>
      ...
      {expanded ? (
        <div className="mt-2 ml-8">
          {safeInput ? (
            <div className="overflow-hidden rounded-lg border border-[var(--color-border)] bg-gray-50 px-3 py-2 font-mono text-[12px] text-ink-700">
