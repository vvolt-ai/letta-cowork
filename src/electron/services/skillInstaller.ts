 ...
 * Uses a marker file to avoid re-downloading on every startup.
 */
export async function ensureDefaultCoworkSkillsInstalled(): Promise<void> {
    ...
    // Check if already installed (by marker file)
    try {
        await fs.access(markerFile);
        console.log("[skillInstaller] default cowork skills already installed");
        return;
    } catch {
        // Not found — continue to install
    }
    ...
    try {
        console.log("[skillInstaller] Installing default cowork skills from GitHub...");
        ...
        console.log(`[skillInstaller] Found ${skillDirs.length} skills to install:`, skillDirs.map((s) => s.name).join(", "));

        // Download each skill directory separately
        for (const skillDir of skillDirs) {
            const skillHandle = `${DEFAULT_SKILLS_REPO.owner}/${DEFAULT_SKILLS_REPO.repo}/${skillDir.path}`;
            console.log(`[skillInstaller] Installing ${skillDir.name}...`);
            await downloadSkillsFromGitHub(skillHandle, skillDir.name, DEFAULT_SKILLS_REPO.branch);
        }

        console.log("[skillInstaller] Default cowork skills installed successfully");

        // Create marker file to indicate successful installation
        await fs.writeFile(markerFile, new Date().toISOString(), "utf-8");
    } catch (err) {
        console.warn("[skillInstaller] Failed to install default cowork skills:", err);
    }
}
